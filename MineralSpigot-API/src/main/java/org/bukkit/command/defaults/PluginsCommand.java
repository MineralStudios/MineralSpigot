...
public class PluginsCommand extends BukkitCommand {
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (sender instanceof Player player) {
            player.sendMessage(this.getPluginListComponents(player.getHiddenPlugins(),
                    sender.hasPermission("bukkit.command.version")));
            return true;
        }
        // PandaSpigot end
     ...
     */
    private BaseComponent[] getPluginListComponents(List<String> hiddenPlugins, boolean versionCommand) {
        // Use a TreeMap for sorting
        TreeMap<String, Plugin> plugins = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);

        for (Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
            val name = plugin.getDescription().getName();
