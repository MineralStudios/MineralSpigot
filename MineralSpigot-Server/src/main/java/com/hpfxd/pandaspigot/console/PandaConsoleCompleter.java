...
public class PandaConsoleCompleter implements Completer {
    ...
    public void complete(LineReader reader, ParsedLine line, List<Candidate> candidates) {
        ...
        event = new com.destroystokyo.paper.event.server.AsyncTabCompleteEvent(server.server.getConsoleSender(),
                completions, line.line(), true, null);
        event.callEvent();
        ...
        if (event.isCancelled() || event.isHandled()) {
            ...
            if (!event.isCancelled() && TabCompleteEvent.getHandlerList().getRegisteredListeners().length > 0) {
                ...
                Waitable<List<String>> syncCompletions = new Waitable<List<String>>() {
                    ...
                    protected List<String> evaluate() {
                        org.bukkit.event.server.TabCompleteEvent syncEvent = new org.bukkit.event.server.TabCompleteEvent(
                                server.server.getConsoleSender(), line.line(), finalCompletions);
                        return syncEvent.callEvent() ? syncEvent.getCompletions()
            ...
            if (!completions.isEmpty()) {
                candidates
                        .addAll(completions.stream().map(Candidate::new).collect(java.util.stream.Collectors.toList()));
            }
            return;
        }
    ...
    private class CompletionWaiter extends Waitable<List<String>> {
        ...
        protected List<String> evaluate() {
            ...
            TabCompleteEvent tabEvent = new TabCompleteEvent(server.getConsoleSender(), buffer,
                    offers == null ? Collections.emptyList() : offers);
