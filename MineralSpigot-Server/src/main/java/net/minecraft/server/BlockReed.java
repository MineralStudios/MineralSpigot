    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        if (world.getType(blockposition.down()).getBlock() == Blocks.REEDS
            ...
            if (world.isEmpty(blockposition.up())) {
                ...
                if (i < GlobalConfig.getInstance().getReedMaxHeight()) { // PaperSpigot - Configurable max growth height
                                                                         // for reed blocks) {
                    int j = ((Integer) iblockdata.get(BlockReed.AGE)).intValue();

                    if (j >= (byte) range(3,
                            (world.growthOdds / GlobalConfig.getInstance().getCaneModifier() * 15) + 0.5F, 15)) { // Spigot
                        // CraftBukkit start
