...
public class Chunk {
    ...
    public PacketPlayOutMapChunk.ChunkMap getChunkMap(boolean groundUpContinuous, int primaryBitMask) {
        if (!GlobalConfig.getInstance().isCacheChunkMaps() || !groundUpContinuous
    ...
    private void a(int i, int j, int k, int l) {
        if (l > k && this.world.areChunksLoaded(new BlockPosition(i, 0, j), 16)) {
            for (int i1 = k; i1 < l; ++i1) {
                this.world.updateBrightness(EnumSkyBlock.SKY, new BlockPosition(i, i1, j), this); // PaperSpigot -
                                                                                                  // Asynchronous
                                                                                                  // lighting updates //
                                                                                                  // PandaSpigot -
                                                                                                  // Optimize Light
    ...
    public void a(BlockPosition blockposition, TileEntity tileentity) {
        ...
        if (this.getType(blockposition) instanceof IContainer) {
        ...
        } else if (GlobalConfig.getInstance().isRemoveInvalidMobSpawnerTEs()
            ...
            ServerInternalException e = new ServerInternalException(
                    "Attempted to place a tile entity (" + tileentity + ") at " + tileentity.position.getX() + ","
                            ...
                            + " (" + CraftMagicNumbers.getMaterial(getType(blockposition))
    ...
    public void a(Entity entity, AxisAlignedBB axisalignedbb, List<Entity> list, Predicate<? super Entity> predicate) {
        ...
        for (int k = i; k <= j; ++k) {
            if (!this.entitySlices[k].isEmpty()) {
                 ...
                 * It should be the one that checks `var1 instanceof IInventory &&
                 * var1.isAlive()`
                 */
                if (predicate == IEntitySelector.c && inventoryEntityCounts[k] <= 0)
                    continue;
                // PaperSpigot end
                while (iterator.hasNext()) {
                    try {
                        ...
                        if (entity1.getBoundingBox().b(axisalignedbb) && entity1 != entity) {
                            ...
                            if (aentity != null) {
                                for (int l = 0; l < aentity.length; ++l) {
                                    entity1 = aentity[l];
                                    if (entity1 != entity && entity1.getBoundingBox().b(axisalignedbb)
    ...
    public void b(boolean flag) {
        ...
        this.p = true;
        if (!this.lit && this.done && GlobalConfig.getInstance().isRandomLightUpdates()) { // Spigot - also use random
                                                                                           // light
    ...
    public boolean isReady() {
         ...
         * lit are sent to the client.
         * Unfortunately this interferes with our modified chunk ticking algorithm,
         * which will only tick chunks distant from the player on a very infrequent
         * basis.
         * We cannot unfortunately do this lighting stage during chunk gen as it appears
         * to put a lot more noticeable load on the server, than when it is done at play
         * time.
         * For now at least we will simply send all chunks, in accordance with pre 1.7
         * behaviour.
         */
        return true;
        // Spigot End
    ...
    public void n() {
        ...
        if (!this.world.worldProvider.o()) {
            if (this.world.areChunksLoadedBetween(blockposition.a(-1, 0, -1),
                    blockposition.a(16, this.world.F(), 16))) {
