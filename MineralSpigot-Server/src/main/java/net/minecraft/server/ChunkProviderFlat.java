...
public class ChunkProviderFlat implements IChunkProvider {
    ...
    public ChunkProviderFlat(World world, long i, boolean flag, String s) {
        ...
        if (flag) {
            ...
            if (map.containsKey("village") && GlobalConfig.getInstance().isGenerateVillage()) { // PaperSpigot
            ...
            if (map.containsKey("biome_1") && GlobalConfig.getInstance().isGenerateTemple()) { // PaperSpigot
                this.e.add(new WorldGenLargeFeature((Map) map.get("biome_1")));
            }

            if (map.containsKey("mineshaft") && GlobalConfig.getInstance().isGenerateMineshaft()) { // PaperSpigot
                this.e.add(new WorldGenMineshaft((Map) map.get("mineshaft")));
            }

            if (map.containsKey("stronghold") && GlobalConfig.getInstance().isGenerateStronghold()) { // PaperSpigot
                this.e.add(new WorldGenStronghold((Map) map.get("stronghold")));
            }

            if (map.containsKey("oceanmonument") && GlobalConfig.getInstance().isGenerateMonument()) { // PaperSpigot
        ...
        this.g = GlobalConfig.getInstance().isGenerateDungeon() && this.d.b().containsKey("dungeon"); // PaperSpigot
        int j = 0;
        int k = 0;
        boolean flag1 = true;
