...
public class DataWatcher {
    ...
    // Spigot End
    private boolean e;
    // private ReadWriteLock f = new ReentrantReadWriteLock(); // PandaSpigot -
    ...
    public <T> void a(int i, T t0) {
        if (registrationLocked)
            throw new IllegalStateException("Registering datawatcher object after entity initialization"); // PandaSpigot
    ...
    private DataWatcher.WatchableObject j(int i) {
         ...
         * 
         * try {
         * datawatcher_watchableobject = (DataWatcher.WatchableObject)
         * this.dataValues.get(i); // Spigot
    ...
    public static void a(List<DataWatcher.WatchableObject> list, PacketDataSerializer packetdataserializer)
            throws IOException {
        if (list != null) {
    ...
    private static void a(PacketDataSerializer packetdataserializer,
            DataWatcher.WatchableObject datawatcher_watchableobject) throws IOException {
            ...
            switch (i) {
                case 0:
                    datawatcher_watchableobject = new DataWatcher.WatchableObject(i, j,
                            Byte.valueOf(packetdataserializer.readByte()));
                    break;

                case 1:
                    datawatcher_watchableobject = new DataWatcher.WatchableObject(i, j,
                            Short.valueOf(packetdataserializer.readShort()));
                    break;

                case 2:
                    datawatcher_watchableobject = new DataWatcher.WatchableObject(i, j,
                            Integer.valueOf(packetdataserializer.readInt()));
                    break;

                case 3:
                    datawatcher_watchableobject = new DataWatcher.WatchableObject(i, j,
                            Float.valueOf(packetdataserializer.readFloat()));
                    break;
