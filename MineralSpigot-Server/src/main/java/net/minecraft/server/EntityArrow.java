...
public class EntityArrow extends Entity implements IProjectile {
    ...
    public EntityArrow(World world, EntityLiving entityliving, float f) {
        ...
        this.setSize(0.5F, 0.5F);
        this.setPositionRotation(entityliving.locX, entityliving.locY + (double) entityliving.getHeadHeight(),
                entityliving.locZ, entityliving.yaw, entityliving.pitch);
        this.locX -= (double) (MathHelper.cos(this.yaw / 180.0F * 3.1415927F) * 0.16F);
        this.locY -= 0.10000000149011612D;
        this.locZ -= (double) (MathHelper.sin(this.yaw / 180.0F * 3.1415927F) * 0.16F);
        this.setPosition(this.locX, this.locY, this.locZ);
        this.motX = (double) (-MathHelper.sin(this.yaw / 180.0F * 3.1415927F)
                * MathHelper.cos(this.pitch / 180.0F * 3.1415927F));
        this.motZ = (double) (MathHelper.cos(this.yaw / 180.0F * 3.1415927F)
                * MathHelper.cos(this.pitch / 180.0F * 3.1415927F));
        this.motY = (double) (-MathHelper.sin(this.pitch / 180.0F * 3.1415927F));
        this.shoot(this.motX, this.motY, this.motZ, f * 1.5F,
                GlobalConfig.getInstance().getArrowTrajectoryRandomness()); // PandaSpigot
    }
    ...
    public void shoot(double d0, double d1, double d2, float f, float f1) {
        ...
        if (f1 != 0) { // PandaSpigot - Configurable random arrow trajectory
            d0 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1) * 0.007499999832361937D
                    * (double) f1;
            d1 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1) * 0.007499999832361937D
                    * (double) f1;
            d2 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1) * 0.007499999832361937D
                    * (double) f1;
    ...
    public void t_() {
        ...
        if (this.inGround) {
            ...
            if (block == this.g && i == this.h) {
                ++this.ar;
                if (this.ar >= GlobalConfig.getInstance().getArrowDespawnRate()) { // Spigot - First int after shooter
                    this.die();
                }
            } else {
                this.inGround = false;
            ...
            if (movingobjectposition != null && movingobjectposition.entity != null) {
                com.destroystokyo.paper.event.entity.ProjectileCollideEvent event = org.bukkit.craftbukkit.event.CraftEventFactory
                        .callProjectileCollideEvent(this, movingobjectposition);
            ...
            if (movingobjectposition != null) {
                org.bukkit.craftbukkit.event.CraftEventFactory.callProjectileHitEvent(this, movingobjectposition); // CraftBukkit
                                                                                                                   // -
                                                                                                                   // Call
                                                                                                                   // event
                                                                                                                   // //
                ...
                if (movingobjectposition.entity != null) {
                    ...
                    if (this.isCritical() && GlobalConfig.getInstance().isRandomArrowDamage()) { // PandaSpigot
                            ...
                            if (this.knockbackStrength > 0) {
                                ...
                                if (f3 > 0.0F) {
                                    movingobjectposition.entity.g(
                                            this.motX * (double) this.knockbackStrength * 0.6000000238418579D
                                                    / (double) f3,
                                            0.1D, this.motZ * (double) this.knockbackStrength * 0.6000000238418579D
                                                    / (double) f3);
                                }
                            }
            ...
            if (this.isCritical()) {
                for (j = 0; j < 4; ++j) {
                    this.world.addParticle(EnumParticle.CRIT, this.locX + this.motX * (double) j / 4.0D,
                            this.locY + this.motY * (double) j / 4.0D, this.locZ + this.motZ * (double) j / 4.0D,
                            -this.motX, -this.motY + 0.2D, -this.motZ, new int[0]);
                }
            }
            ...
            for (this.pitch = (float) (MathHelper.b(this.motY, (double) f2) * 180.0D / 3.1415927410125732D); this.pitch
            ...
            if (this.V()) {
                for (int l = 0; l < 4; ++l) {
                    f3 = 0.25F;
                    this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX - this.motX * (double) f3,
                            this.locY - this.motY * (double) f3, this.locZ - this.motZ * (double) f3, this.motX,
    ...
    public void d(EntityHuman entityhuman) {
        if (!this.world.isClientSide && this.inGround && this.shake <= 0) {
            ...
            if (this.fromPlayer == 1 && entityhuman.inventory.canHold(itemstack) > 0) {
                EntityItem item = shooter instanceof EntityHuman
                        ? new EntityItem(this.world, this.locX, this.locY, this.locZ, itemstack,
                                (EntityHuman) this.shooter)
                        : new EntityItem(this.world, this.locX, this.locY, this.locZ, itemstack);
                ...
                org.bukkit.event.entity.EntityPickupItemEvent entityEvent = new org.bukkit.event.entity.EntityPickupItemEvent(
                        entityhuman.getBukkitEntity(), event.getItem(), 0);
                // event.setCancelled(!entityhuman.canPickUpLoot); TODO
