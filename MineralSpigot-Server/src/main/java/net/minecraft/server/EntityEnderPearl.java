...
public class EntityEnderPearl extends EntityProjectile {
    ...
    public EntityEnderPearl(World world) {
        super(world);
        this.loadChunks = GlobalConfig.getInstance().isLoadUnloadedEnderPearls(); // PaperSpigot
    }
    ...
    protected void a(MovingObjectPosition movingobjectposition) {
        ...
        if (this.inUnloadedChunk && GlobalConfig.getInstance().isRemoveUnloadedEnderPearls()) {
            this.die();
        }
        // PaperSpigot end
        ...
        for (int i = 0; i < 32; ++i) {
            this.world.addParticle(EnumParticle.PORTAL, this.locX, this.locY + this.random.nextDouble() * 2.0D,
                    this.locZ, this.random.nextGaussian(), 0.0D, this.random.nextGaussian(), new int[0]);
        }
            ...
            if (entityliving instanceof EntityPlayer) {
                ...
                if (entityplayer.playerConnection.a().g() && entityplayer.world == this.world
                    ...
                    if (!teleEvent.isCancelled() && !entityplayer.playerConnection.isDisconnected()) {
                        ...
                        LagCompensator.INSTANCE.registerMovement(player, location); // Nacho
