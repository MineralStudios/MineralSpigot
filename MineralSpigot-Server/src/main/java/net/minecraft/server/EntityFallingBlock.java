...
public class EntityFallingBlock extends Entity {
    ...
    public EntityFallingBlock(org.bukkit.Location loc, World world) {
        super(world);
        sourceLoc = loc;
        this.loadChunks = GlobalConfig.getInstance().isLoadUnloadedFallingBlocks(); // PaperSpigot
    }
    ...
    public void t_() {
        ...
        if (block.getMaterial() == Material.AIR) {
            ...
            if (this.inUnloadedChunk && GlobalConfig.getInstance().isRemoveUnloadedFallingBlocks()) {
                this.die();
            }
            // PaperSpigot end

            // PaperSpigot start - Drop falling blocks above the specified height
            if (GlobalConfig.getInstance().getFallingBlockHeightNerf() != 0
                    && this.locY > GlobalConfig.getInstance().getFallingBlockHeightNerf()) {
                    ...
                    if (this.world.getType(blockposition).getBlock() != Blocks.PISTON_EXTENSION) {
                        ...
                        if (!this.e) {
                            if (this.world.a(block, blockposition, true, EnumDirection.UP, (Entity) null,
                                ...
                                GlobalConfig.getInstance().getAntiXrayInstance().updateNearbyBlocks(world,
                                        blockposition); // Spigot
                                // CraftBukkit end
            ...
            if (i > 0) {
                ...
                while (iterator.hasNext()) {
                    ...
                    entity.damageEntity(damagesource,
                            (float) Math.min(MathHelper.d((float) i * this.fallHurtAmount), this.fallHurtMax));
    ...
    public void appendEntityCrashDetails(CrashReportSystemDetails crashreportsystemdetails) {
        ...
        if (this.block != null) {
            ...
            crashreportsystemdetails.a("Immitating block data",
                    (Object) Integer.valueOf(block.toLegacyData(this.block)));
        }
    ...
    public double f(double d0, double d1, double d2) {
        if (!GlobalConfig.getInstance().isFixCannons())
    ...
    public float getHeadHeight() {
        return GlobalConfig.getInstance().isFixCannons() ? this.length / 2 : super.getHeadHeight();
    }
    // PaperSpigot end
}
