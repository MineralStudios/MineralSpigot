...
public abstract class EntityHuman extends EntityLiving {
    ...
    public EntityHuman(World world, GameProfile gameprofile) {
        ...
        this.setPositionRotation((double) blockposition.getX() + 0.5D, (double) (blockposition.getY() + 1),
    ...
    protected void s() {
        if (this.g != null) {
            ...
            if (event.isCancelled()) {
                ...
                if (this instanceof EntityPlayer) {
                    ((EntityPlayer) this).playerConnection.sendPacket(new PacketPlayOutSetSlot((byte) 0,
                            activeContainer.getSlot((IInventory) this.inventory, this.inventory.itemInHandIndex).index,
                            this.g));
                    // Spigot Start
            ...
            if (event.getReplacement() != null) {
                ...
                // If the replacement was AIR, then itemstack will be null, which will cause an
                // NPE later,
                // so instead we use the default replacement with zero count, which is handled
                // properly.
                if (itemstack == null) {
            ...
            if (itemstack != defaultReplacement && this instanceof EntityPlayer) {
                ((EntityPlayer) this).playerConnection.sendPacket(new PacketPlayOutSetSlot((byte) 0, activeContainer
                        .getSlot((IInventory) this.inventory, this.inventory.itemInHandIndex).rawSlotIndex, itemstack));
            }
            // CraftBukkit end
    ...
    public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
        if (itemstack == null) {
            ...
            EntityItem entityitem = new EntityItem(this.world, this.locX, d0, this.locZ, itemstack, this);
            ...
            if (flag) {
                ...
                f = 0.3F;
                entityitem.motX = (double) (-MathHelper.sin(this.yaw / 180.0F * 3.1415927F)
                        * MathHelper.cos(this.pitch / 180.0F * 3.1415927F) * f);
                entityitem.motZ = (double) (MathHelper.cos(this.yaw / 180.0F * 3.1415927F)
                        * MathHelper.cos(this.pitch / 180.0F * 3.1415927F) * f);
    ...
    protected boolean d(DamageSource damagesource, float f) { // void -> boolean
        ...
        if (!this.isInvulnerable(damagesource)) {
            if (!damagesource.ignoresArmor() && this.isBlocking() && f > 0.0F) {
                f = (1.0F + f) * GlobalConfig.getInstance().getPlayerBlockingDamageMultiplier(); // PaperSpigot -
                // Configurable damage
                // multiplier for
                // blocking;
            }
        ...
        if (playerAttackEntityEvent.callEvent() && willAttack) { // Logic moved to willAttack local variable.
            {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    boolean flag = !GlobalConfig.getInstance().isDisablePlayerCrits() && this.fallDistance > 0.0F
                    ...
                    if (flag2) {
                        if (i > 0) {
                            ...
                            if (entity instanceof EntityLiving victim) {
                                val kbProfile = victim.kbProfile != null ? victim.kbProfile
                                        : victim.maxNoDamageTicks < 10 ? KnockbackProfileList.getComboKnockbackProfile()
                                                : KnockbackProfileList.getDefaultKnockbackProfile();
                                kbProfile.callSecondStage(
                                        this, victim, EnchantmentManager.a((EntityLiving) this));
                            } else {
                        ...
                        if (entity instanceof EntityPlayer && entity.velocityChanged) {
                            ...
                            val attackedPlayer = (EntityPlayer) entity;
                            val event = new PlayerVelocityEvent(attackedPlayer.getBukkitEntity(),
                        ...
                        if (entity instanceof EntityLiving living)
                            EnchantmentManager.a(living, (Entity) this);
                        ...
                        this.applyExhaustion(GlobalConfig.getInstance().getCombatExhaustion()); // Spigot - Change to
                                                                                                // use
                        // configurable value
                    } else if (flag1)
                        entity.extinguish();
    ...
    public EntityHuman.EnumBedResult a(BlockPosition blockposition) {
        ...
        if (this.world.isLoaded(blockposition)) {
            EnumDirection enumdirection = (EnumDirection) this.world.getType(blockposition)
            ...
            this.setPosition((double) ((float) blockposition.getX() + f),
                    (double) ((float) blockposition.getY() + 0.6875F), (double) ((float) blockposition.getZ() + f1));
        } else
            this.setPosition((double) ((float) blockposition.getX() + 0.5F),
                    (double) ((float) blockposition.getY() + 0.6875F), (double) ((float) blockposition.getZ() + 0.5F));
    ...
    public void a(boolean flag, boolean flag1, boolean flag2) {
        ...
        if (this.bx != null && iblockdata.getBlock() == Blocks.BED) {
            ...
            this.setPosition((double) ((float) blockposition.getX() + 0.5F),
                    (double) ((float) blockposition.getY() + 0.1F), (double) ((float) blockposition.getZ() + 0.5F));
        }
    ...
    public void bF() {
        ...
        if (this.isSprinting()) {
            this.applyExhaustion(GlobalConfig.getInstance().getSprintExhaustion()); // Spigot - Change to use
                                                                                    // configurable
            // value
        } else {
            this.applyExhaustion(GlobalConfig.getInstance().getWalkExhaustion()); // Spigot - Change to use configurable
            // value
        }
        ...
        if (this.vehicle == null) {
            ...
            if (this.a(Material.WATER)) {
                ...
                if (i > 0) {
                    ...
                    this.applyExhaustion(GlobalConfig.getInstance().getPlayerSwimmingExhaustion() * (float) i * 0.01F); // PaperSpigot
                    // -
                    // Configurable
                    // swimming
                    // exhaustion
        ...
        if (this.vehicle != null) {
            ...
            if (i > 0) {
                if (this.vehicle instanceof EntityMinecartAbstract) {
                    ...
                    } else if (this.e.c((double) MathHelper.floor(this.locX), (double) MathHelper.floor(this.locY),
                            (double) MathHelper.floor(this.locZ)) >= 1000000.0D) {
    ...
    public void a(EntityLiving entityliving) {
        ...
        EntityTypes.MonsterEggInfo entitytypes_monsteregginfo = (EntityTypes.MonsterEggInfo) EntityTypes.eggInfo
