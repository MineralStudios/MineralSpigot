...
public abstract class EntityLiving extends Entity {
    ...
    public final Int2ObjectOpenHashMap<MobEffect> effects = new Int2ObjectOpenHashMap<>();
    ...
    public boolean canHit() {
        return noDamageTicks <= maxNoDamageTicks / 2.0F;
    }
    ...
    public void K() {
        ...
        if (this.isAlive()) {
            if (this.inBlock()) {
                ...
                double d0 = this.world.getWorldBorder().a((Entity) this)
                        + this.world.getWorldBorder().getDamageBuffer();

                if (d0 < 0.0D) {
                    this.damageEntity(DamageSource.STUCK,
                            (float) Math.max(1, MathHelper.floor(-d0 * this.world.getWorldBorder().getDamageAmount())));
                }
            }
        }
        ...
        if (i > 0) {
            ...
            if (flag1 && i > 0) {
                ...
                this.world.addParticle(flag ? EnumParticle.SPELL_MOB_AMBIENT : EnumParticle.SPELL_MOB,
                        ...
                        this.locZ + (this.random.nextDouble() - 0.5D) * (double) this.width, d0, d1, d2, new int[0]);
            }
        }
        ...
        if (!this.ar || this.as >= this.n() / 2 || this.as < 0) {
            ...
            if (this.world instanceof WorldServer) {
                ((WorldServer) this.world).getTracker().a((Entity) this,
                        (Packet) (new PacketPlayOutAnimation(this, 0)));
            }
        }
    ...
    public void m() {
        ...
        if (((WorldServer) this.world).hasEntityMoveEvent && !(this instanceof EntityPlayer)) {
            if (this.lastX != this.locX || this.lastY != this.locY || this.lastZ != this.locZ
                    || this.lastYaw != this.yaw || this.lastPitch != this.pitch) {
                org.bukkit.Location from = new org.bukkit.Location(this.world.getWorld(), this.lastX, this.lastY,
                        this.lastZ, this.lastYaw, this.lastPitch);
                org.bukkit.Location to = new org.bukkit.Location(this.world.getWorld(), this.locX, this.locY, this.locZ,
                        this.yaw, this.pitch);
                io.papermc.paper.event.entity.EntityMoveEvent event = new io.papermc.paper.event.entity.EntityMoveEvent(
                        (LivingEntity) this.getBukkitEntity(), from, to.clone());
                if (!event.callEvent()) {
                    // if cancelled, set back to previous position
                    this.setLocation(from.getX(), from.getY(), from.getZ(), from.getYaw(), from.getPitch());
                } else if (!to.equals(event.getTo())) {
                    this.setLocation(event.getTo().getX(), event.getTo().getY(), event.getTo().getZ(),
    ...
    protected void bL() {
        if (GlobalConfig.getInstance().getMaxCollisionsPerEntity() == 0)
            return; // Mineral
        List<Entity> list = this.world.a(this,
                this.getBoundingBox().grow(0.20000000298023224D, 0.0D, 0.20000000298023224D),
                Predicates.and(IEntitySelector.d, new Predicate<Entity>() {
        ...
        if (this.ad() && !list.isEmpty()) { // Spigot: Add this.ad() condition
            numCollisions -= GlobalConfig.getInstance().getMaxCollisionsPerEntity(); // Spigot
            for (int i = 0; i < list.size(); ++i) {
                if (numCollisions > GlobalConfig.getInstance().getMaxCollisionsPerEntity()) {
                    break;
                } // Spigot
