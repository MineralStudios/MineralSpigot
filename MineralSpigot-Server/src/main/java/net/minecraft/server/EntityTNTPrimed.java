...
public class EntityTNTPrimed extends Entity {
    ...
    public EntityTNTPrimed(org.bukkit.Location loc, World world) {
        ...
        this.loadChunks = GlobalConfig.getInstance().isLoadUnloadedTNTEntities(); // PaperSpigot
    }
    ...
    public void t_() {
        if (GlobalConfig.getInstance().currentPrimedTnt++ > GlobalConfig.getInstance().getMaxTntTicksPerTick()) {
            return;
        } // Spigot
        ...
        if (GlobalConfig.getInstance().getTntEntityHeightNerf() != 0
                && this.locY > GlobalConfig.getInstance().getTntEntityHeightNerf()) {
            this.die();
        }
        // PaperSpigot end

        // PaperSpigot start - Remove entities in unloaded chunks
        if (this.inUnloadedChunk && GlobalConfig.getInstance().isRemoveUnloadedTNTEntities()) {
            this.die();
            this.fuseTicks = 2;
        }
        // PaperSpigot end
    ...
    private void explode() {
        ...
        if (GlobalConfig.getInstance().isLoadUnloadedTNTEntities()) {
    ...
    @Override
    public boolean aL() {
        return !GlobalConfig.getInstance().isFixCannons() && super.aL();
    }

    @Override
    public float getHeadHeight() {
        return GlobalConfig.getInstance().isFixCannons() ? this.length / 2 : 0.0F;
    }
