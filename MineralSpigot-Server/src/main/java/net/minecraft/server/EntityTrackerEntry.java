...
public class EntityTrackerEntry {
    ...
    public void track(List<EntityHuman> list) {
        ...
        if (this.tracker instanceof EntityItemFrame /* && this.m % 10 == 0 */) { // CraftBukkit - Moved below, should
                                                                                 // always enter this block
            EntityItemFrame entityitemframe = (EntityItemFrame) this.tracker;
            ItemStack itemstack = entityitemframe.getItem();

            if (this.m % 10 == 0 && itemstack != null && itemstack.getItem() instanceof ItemWorldMap) { // CraftBukkit -
                                                                                                        // Moved this.m
                                                                                                        // % 10 logic
                                                                                                        // here so item
                                                                                                        // frames do not
        ...
        if (this.tracker.ai || isFishingHook || this.m % this.c == 0) {
            ...
            if (this.tracker.vehicle == null) {
                ...
                if (this.receiveRelativeMovementUpdates && this.m > 0) { // PaperSpigot - Moved up
                    ...
                    if (j1 >= -128 && j1 < 128 && k1 >= -128 && k1 < 128 && l1 >= -128 && l1 < 128 && this.v <= 400
                            && !this.x && this.y == this.tracker.onGround) {
        ...
        if (entityplayer != this.tracker) {
            if (this.c(entityplayer)) {
                if (!this.trackedPlayers.contains(entityplayer)
                    ...
                    entityplayer.playerConnection
                            .sendPacket(new PacketPlayOutEntityHeadRotation(this.tracker, (byte) this.i)); // PandaSpigot
                                                                                                           // - Don't
                                                                                                           // broadcast
                    // CraftBukkit end
    ...
    public void scanPlayers(List<EntityHuman> list) {
        for (EntityHuman entityHuman : list) {
            ...
            if (entityHuman instanceof EntityPlayer && entityHuman.getBukkitEntity() != null
                    && !((EntityPlayer) entityHuman).getBukkitEntity().canSee((this.tracker).getBukkitEntity())) {
                continue;
            }
