...
public class EntityZombie extends EntityMonster {
    ...
    private static final AttributeModifier c = new AttributeModifier(EntityZombie.b, "Baby speed boost",
            GlobalConfig.getInstance().getBabyZombieMovementSpeed(), 1); // PaperSpigot - Configurable baby zombie
                                                                         // movement speed
    ...
    protected void n() {
        if (GlobalConfig.getInstance().isZombieAggressiveTowardsVillager())
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (super.damageEntity(damagesource, f)) {
            ...
            if (entityliving != null && this.world.getDifficulty() == EnumDifficulty.HARD
                    && (double) this.random.nextFloat() < this.getAttributeInstance(EntityZombie.a).getValue()) {
                ...
                for (int l = 0; l < 50; ++l) {
                    ...
                    if (World.a((IBlockAccess) this.world, new BlockPosition(i1, j1 - 1, k1))
                            && !this.world.isLightLevel(new BlockPosition(i1, j1, k1), 10)) { // PandaSpigot - Use
                                                                                              // isLightLevel
                        entityzombie.setPosition((double) i1, (double) j1, (double) k1);
                        if (!this.world.isPlayerNearby((double) i1, (double) j1, (double) k1, 7.0D)
                                && this.world.a(entityzombie.getBoundingBox(), (Entity) entityzombie)
