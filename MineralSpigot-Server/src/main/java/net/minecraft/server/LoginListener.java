...
public class LoginListener implements PacketLoginInListener, IUpdatePlayerListBox {
    ...
    public String d() {
        // PandaSpigot start
        String ip = GlobalConfig.getInstance().isLogPlayerIpAddresses()
                ? this.networkManager.getSocketAddress().toString()
                : "<ip address withheld>";
            ...
            authenticatorPool.execute(new Runnable() { // PandaSpigot - Cache authenticator threads
                ...
                public void run() {
                    try {
                        ...
                        server.server.getLogger().log(java.util.logging.Level.WARNING,
                                "Exception verifying " + i.getName(), ex);
                    }
                }
            ...
            authenticatorPool.execute(new Runnable() { // PandaSpigot - Cache authenticator threads
                public void run() {
                    ...
                    try {
                        ...
                        disconnect("Failed to verify username!");
                        server.server.getLogger().log(java.util.logging.Level.WARNING,
                                "Exception verifying " + gameprofile.getName(), exception);
                        // CraftBukkit end
                    }
    ...
    public class LoginHandler {
        ...
        public void fireEvents() throws Exception {
            ...
            com.destroystokyo.paper.profile.PlayerProfile profile = com.destroystokyo.paper.profile.CraftPlayerProfile
                    .asBukkitCopy(i);
