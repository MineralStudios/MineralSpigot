...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    // PandaSpigot - Remove packet queue locking
    ...
    private boolean o;
    private boolean p;
    private static boolean enableExplicitFlush = Boolean.getBoolean("paper.explicit-flush"); // PandaSpigot - Disable
    // explicit flushing
    ...
    protected final com.hpfxd.pandaspigot.util.IntervalledCounter allPacketCounts = GlobalConfig.getInstance()
            .getAllPacketsLimit() != null ? new com.hpfxd.pandaspigot.util.IntervalledCounter(
            (long) (GlobalConfig.getInstance()
                    .getAllPacketsLimit().packetLimitInterval * 1.0e9))
            : null;
    ...
    private void killForPacketSpam() {
        IChatBaseComponent[] reason = org.bukkit.craftbukkit.util.CraftChatMessage
                .fromString(org.bukkit.ChatColor.translateAlternateColorCodes('&',
    ...
    public void a(ChannelHandlerContext channelhandlercontext, Packet packet) throws Exception {
        if (this.channel.isOpen()) {
            ...
            if (this.allPacketCounts != null ||
                    GlobalConfig.getInstance().getPacketSpecificLimits()
                            .containsKey(packet.getClass())) {
                long time = System.nanoTime();
                synchronized (PACKET_LIMIT_LOCK) {
                    if (this.allPacketCounts != null) {
                        this.allPacketCounts.updateAndAdd(1, time);
                        if (this.allPacketCounts
                                .getRate() >= GlobalConfig.getInstance()
                                .getAllPacketsLimit().maxPacketRate) {
                    ...
                    for (Class<?> check = packet.getClass(); check != Object.class; check = check.getSuperclass()) {
                        PacketLimit packetSpecificLimit = GlobalConfig.getInstance()
                                .getPacketSpecificLimits().get(check);
                        if (packetSpecificLimit == null) {
                            continue;
                        }
                        com.hpfxd.pandaspigot.util.IntervalledCounter counter = this.packetSpecificLimits
                                .computeIfAbsent((Class) check, clazz -> {
                                    return new com.hpfxd.pandaspigot.util.IntervalledCounter(
            ...
            if (packetListener instanceof PlayerConnection playerConnection) {
                ...
                if ((currentDelay > 0 || !backtrackSystem.getPacketReadTasks().isEmpty())
                        && !(packet instanceof PacketPlayInKeepAlive || packet instanceof PacketPlayInTabComplete))
    ...
    private static class InnerUtil { // Attempt to hide these methods from ProtocolLib so it doesn't accidently pick
        // them up.
    ...
    public void a(Packet packet, GenericFutureListener<? extends Future<? super Void>> genericfuturelistener,
        ...
        if (genericfuturelistener != null || agenericfuturelistener != null) { // cannot call ArrayUtils.add with both
            // null arguments
        ...
        if (!hasExtraPackets) {
            ...
            packets.add(new NetworkManager.QueuedPacket(packet,
                    (GenericFutureListener<? extends Future<? super Void>>) null)); // delay the future listener until
            // the end of the extra packets

            for (int i = 0, len = extraPackets.size(); i < len; ) {
                Packet extra = extraPackets.get(i);
                boolean end = ++i == len;
                packets.add(new NetworkManager.QueuedPacket(extra, end ? listeners : null)); // append listener to the
                // end
            }
    ...
    private void dispatchPacket(Packet<?> packet,
        ...
        this.a(packet, agenericfuturelistener);
    ...
    private void writePacket(Packet packet,
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            try {
                // PandaSpigot end
                ChannelFuture channelfuture = (flush) ? this.channel.writeAndFlush(packet) : this.channel.write(packet); // PandaSpigot
                // -
                // add
                // flush
                // parameter
                ...
                if (packet.hasFinishListener()) {
                    channelfuture.addListener((ChannelFutureListener) channelFuture -> packet
                            .onPacketDispatchFinish(player, channelFuture));
                }
                // PandaSpigot end
            ...
            Runnable command = () -> { // PandaSpigot - optimise packets that are not flushed
                ...
                try {
                    // PandaSpigot end
                    ChannelFuture channelfuture = (flush) ? NetworkManager.this.channel.writeAndFlush(packet)
                            : NetworkManager.this.channel.write(packet); // PandaSpigot - add flush parameter
    ...
    private boolean processQueue() {
        ...
        // queue off main anymore
        // But if we are not on main due to login/status, the parent is synchronized on
        // packetQueue
        java.util.Iterator<QueuedPacket> iterator = this.i.iterator();
        while (iterator.hasNext()) {
            NetworkManager.QueuedPacket queued = iterator.next(); // poll -> peek
            // Fix NPE (Spigot bug caused by handleDisconnection())
            if (false && queued == null) { // PandaSpigot - diff on change, this logic is redundant: iterator guarantees
                // ret of an element - on change, hook the flush logic here
                return true;
            }
            ...
            if (!packet.isReady()) {
                ...
                iterator.remove();
                // PandaSpigot start - make only one flush call per sendPacketQueue() call
                this.writePacket(packet, queued.getGenericFutureListeners(),
    ...
    public void a() {
        ...
        if (enableExplicitFlush)
            this.channel.eventLoop().execute(() -> this.channel.flush()); // PandaSpigot - we don't need to explicit
        // flush here, but allow opt-in in case issues
        // are found to a better version
    }
    ...
    public void l() {
        if (this.channel != null && !this.channel.isOpen()) {
            if (!this.p) {
                ...
                // NetworkManager.g.warn("handleDisconnection() called twice"); // PandaSpigot -
    ...
    static class QueuedPacket {
        ...
        private final GenericFutureListener<? extends Future<? super Void>>[] getGenericFutureListeners() {
            return this.b;
        } // PandaSpigot - OBFHELPER
    ...
    public SocketAddress getRawAddress() {
        // PandaSpigot start - This can be null in the case of a Unix domain socket, so
        // if it is, fake something
