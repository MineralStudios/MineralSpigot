...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
    ...
    private static final Set<PacketPlayOutPosition.EnumPlayerTeleportFlags> TELEPORT_NO_ROT = com.google.common.collect.Sets
            .immutableEnumSet(
    ...
    private <T extends PacketListener> boolean ensureMainThread(final Packet<T> packet)
                        ...
                        if (event.isCancelled()) {
                            ...
                            if (GlobalConfig.getInstance().isSmoothTeleportation()) {
                                this.player.playerConnection.sendPacket(new PacketPlayOutPosition(from.getX(),
                                        from.getY(), from.getZ(), 0, 0, TELEPORT_NO_ROT));
                            } else {
                                // PandaSpigot end
                                this.player.playerConnection.sendPacket(new PacketPlayOutPosition(from.getX(),
                                        from.getY(), from.getZ(), from.getYaw(), from.getPitch(),
                ...
                if (this.checkMovement && !this.player.dead) {
                    ...
                    boolean flag = worldserver
                            .getCubes(this.player,
                                    this.player.getBoundingBox().shrink((double) f4, (double) f4, (double) f4))
                            .isEmpty();
                    ...
                    if (d15 > GlobalConfig.getInstance().getMovedWronglyThreshold() && !this.player.isSleeping()
    ...
    private void internalTeleport(double d0, double d1, double d2, float f, float f1, Set set) {
        // PandaSpigot start - Smooth teleportation
        if (GlobalConfig.getInstance().isSmoothTeleportation() && set.isEmpty()) {
                    ...
                    if (packetplayinblockdig.c() == PacketPlayInBlockDig.EnumPlayerDigType.START_DESTROY_BLOCK) {
                        if (!this.minecraftServer.a(worldserver, blockposition, this.player)
                            ...
                            CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK,
                                    blockposition, packetplayinblockdig.b(), this.player.inventory.getItemInHand());
    ...
    @Getter
    private List<Function<Packet<?>, Boolean>> outgoingPacketListeners = new ArrayList<>();
    ...
    public void sendPacket(final Packet packet) {
        ...
        try {
            ...
            for (Function<Packet<?>, Boolean> packetListener : outgoingPacketListeners)
    ...
    public void a(PacketPlayInHeldItemSlot packetplayinhelditemslot) {
        ...
        if (this.player.dead)
            return;
        if (ensureMainThread(packetplayinhelditemslot))
            return; // Blossom
    ...
    public void a(PacketPlayInChat packetplayinchat) {
        ...
        if (packetplayinchat.a().startsWith("/")) {
            if (ensureMainThread(packetplayinchat))
                return; // Blossom
        }
        // CraftBukkit end
        if (this.player.dead || this.player.getChatFlags() == EntityHuman.EnumChatVisibility.HIDDEN) { // CraftBukkit -
            ...
            for (String exclude : GlobalConfig.getInstance().getSpamExclusions()) {
    ...
    private void handleCommand(String s) {
        ...
        if (GlobalConfig.getInstance().isLogCommands()) // Spigot
                        ...
                        case DEFAULT:
                            ...
                            if (itemstack != null &&
                                    ((itemstack.getItem() == Items.LAVA_BUCKET
                                            && GlobalConfig.getInstance().isStackableLavaBuckets()) ||
                                            (itemstack.getItem() == Items.WATER_BUCKET
                                                    && GlobalConfig.getInstance().isStackableWaterBuckets())
                                            ||
                                            (itemstack.getItem() == Items.MILK_BUCKET
                                                    && GlobalConfig.getInstance().isStackableMilkBuckets()))) {
    ...
    public void a(PacketPlayInTransaction packetplayintransaction) {
        ...
        if (oshort != null && packetplayintransaction.b() == oshort.shortValue()
    ...
    public void a(PacketPlayInUpdateSign packetplayinupdatesign) {
        ...
        if (worldserver.isLoaded(blockposition)) {
            ...
            for (int i = 0; i < aichatbasecomponent.length; ++i) {
                // PandaSpigot start - cap line length - modified clients can send longer data
                // than normal
                ...
                if (line.length() > TileEntitySign.MAX_SIGN_LINE_LENGTH && TileEntitySign.MAX_SIGN_LINE_LENGTH > 0) {
                    int offset = line.codePoints().limit(TileEntitySign.MAX_SIGN_LINE_LENGTH).map(Character::charCount)
                            .sum();
    ...
    public void a(PacketPlayInTabComplete packetplayintabcomplete) {
        ...
        event = new com.destroystokyo.paper.event.server.AsyncTabCompleteEvent(this.getPlayer(), completions,
                buffer, isCommand,
                blockpos != null
                        ? new Location(player.world.getWorld(), blockpos.getX(), blockpos.getY(), blockpos.getZ())
                        : null);
        event.callEvent();
    ...
    private boolean validateBook(ItemStack testStack) {
        ...
        long byteTotal = 0;
        int maxBookPageSize = GlobalConfig.getInstance().getMaxBookPageSize();
        double multiplier = Math.max(0.3D,
                Math.min(1D, GlobalConfig.getInstance().getMaxBookTotalSizeMultiplier()));
        ...
        if (byteTotal > byteAllowed) {
            PlayerConnection.c.warn(this.player.getName() + " tried to send too large of a book. Book Size: "
            ...
            if ("MC|BEdit".equals(packetplayincustompayload.a())) {
                ...
                if (!this.minecraftServer.getEnableCommandBlock()) {
                    ...
                    try {
                        ...
                        if (b0 == 0) {
                            TileEntity tileentity = this.player.world
                                    .getTileEntity(new BlockPosition(packetdataserializer.readInt(),
                ...
                if (packetplayincustompayload.b() != null && packetplayincustompayload.b().readableBytes() >= 1) {
                    String s1 = SharedConstants.a(packetplayincustompayload.b().c(30));
            ...
            // CraftBukkit start
                ...
                try {
                    ...
                    server.getMessenger().dispatchIncomingMessage(player.getBukkitEntity(),
                            packetplayincustompayload.a(), data);
                } catch (Exception ex) {
                    PlayerConnection.c.error("Couldn\'t register custom payload", ex);
