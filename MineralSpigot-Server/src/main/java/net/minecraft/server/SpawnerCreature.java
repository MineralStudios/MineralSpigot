...
public final class SpawnerCreature {
    ...
    private int getEntityCount(WorldServer server, Class oClass) {
        // PandaSpigot start - use entire world, not just active chunks. Spigot broke
        ...
        if (!flag && !flag1) {
            ...
            while (iterator.hasNext()) {
                ...
                if (!entityhuman.isSpectator() || !entityhuman.affectsSpawning) { // PaperSpigot
                    ...
                    byte b0 = 8;
                    // Spigot Start
                    b0 = GlobalConfig.getInstance().getMobSpawnRange();
                    b0 = (b0 > GlobalConfig.getInstance().getViewDistance())
                            ? (byte) GlobalConfig.getInstance().getViewDistance()
                            : b0;
                    b0 = (b0 > 8) ? 8 : b0;
                    // Spigot End
            ...
            for (int k1 = 0; k1 < j; ++k1) {
                ...
                if ((!enumcreaturetype.d() || flag1) && (enumcreaturetype.d() || flag)
                    ...
                    if ((mobcnt = getEntityCount(worldserver, enumcreaturetype.a())) <= limit * i / 289) { // PandaSpigot
                                                                                                           // - use
                                                                                                           // 17x17 like
                                                                                                           // vanilla (a
                                                                                                           // at top of
                        ...
                        int moblimit = (limit * i / 289) - mobcnt + 1; // Spigot - up to 1 more than limit //
                                                                       // PandaSpigot - use 17x17 like vanilla (a at top
                                                                       // of file)
                                            ...
                                            label108: {
                                                ...
                                                if (!worldserver.isPlayerNearbyWhoAffectsSpawning((double) f,
                                                        ...
                                                        && blockposition.c((double) f, (double) k3,
                                                                (double) f1) >= 576.0D) { // PaperSpigot - Affects
                                                                                          // Spawning API
