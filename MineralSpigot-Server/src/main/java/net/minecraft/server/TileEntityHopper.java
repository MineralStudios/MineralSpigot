...
public class TileEntityHopper extends TileEntityContainer implements IHopper, IUpdatePlayerListBox {
    ...
    public boolean m() {
        if (this.world != null && !this.world.isClientSide) {
            if (!this.n() && BlockHopper.f(this.u())) {
                ...
                if (flag) {
                    this.d(GlobalConfig.getInstance().getHopperTransfer()); // Spigot
                    this.update();
                    return true;
                }
            }
            // PaperSpigot start
            if (GlobalConfig.getInstance().isUseHopperCheck() && !this.n()) {
                this.d(GlobalConfig.getInstance().getHopperCheck());
            }
            // PaperSpigot end
            return false;
        } else {
                ...
                for (int i = 0; i < this.getSize(); ++i) {
                    if (this.getItem(i) != null) {
                        ...
                        CraftItemStack oitemstack = CraftItemStack
                                .asCraftMirror(this.splitStack(i, GlobalConfig.getInstance().getHopperAmount())); // Spigot
                        ...
                        ItemStack itemstack1 = addItem(iinventory, CraftItemStack.asNMSCopy(event.getItem()),
                                enumdirection);
    ...
    private static boolean a(IHopper ihopper, IInventory iinventory, int i, EnumDirection enumdirection) {
        ...
        if (itemstack != null && b(iinventory, itemstack, i, enumdirection)) {
            ...
            // the hopper
            CraftItemStack oitemstack = CraftItemStack
                    .asCraftMirror(iinventory.splitStack(i, GlobalConfig.getInstance().getHopperAmount())); // Spigot
            ...
            if (event.isCancelled()) {
                ...
                if (ihopper instanceof TileEntityHopper) {
                    ((TileEntityHopper) ihopper).d(GlobalConfig.getInstance().getHopperTransfer()); // Spigot
                } else if (ihopper instanceof EntityMinecartHopper) {
                    ((EntityMinecartHopper) ihopper).m(GlobalConfig.getInstance().getHopperTransfer() / 2); // Spigot
                }
                return false;
            }
            int origCount = event.getItem().getAmount(); // Spigot
            ...
            if (flag) {
                if (iinventory instanceof TileEntityHopper) {
                    ...
                    if (tileentityhopper.o()) {
                        tileentityhopper.d(GlobalConfig.getInstance().getHopperTransfer()); // Spigot
                    }
