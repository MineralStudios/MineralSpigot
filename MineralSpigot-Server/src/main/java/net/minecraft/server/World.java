...
public abstract class World implements IBlockAccess {
    ...
    public ExecutorService lightingExecutor = Executors
            .newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("PaperSpigot - Lighting Thread").build()); // PaperSpigot
                                                                                                                         ...
                                                                                                                         // lighting
                                                                                                                         // updates
    public final Object2FloatOpenHashMap<Explosion.CacheKey> explosionDensityCache = new Object2FloatOpenHashMap<>(); // PaperSpigot
    // -
    // Optimize
    // explosions
    ...
    protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
        ...
        // CraftBukkit end
        this.keepSpawnInMemory = GlobalConfig.getInstance().isKeepSpawnInMemory(); // PaperSpigot
        this.entityLimiter = new org.spigotmc.TickLimiter(GlobalConfig.getInstance().getEntityMaxTickTime());
        this.tileLimiter = new org.spigotmc.TickLimiter(GlobalConfig.getInstance().getTileMaxTickTime());
    }
    ...
    public void notifyAndUpdatePhysics(BlockPosition blockposition, Chunk chunk, Block oldBlock, Block newBLock,
            int flag) {
        if ((flag & 2) != 0 && (chunk == null || chunk.isReady())) { // allow chunk to be null here as chunk.isReady()
                                                                     // is false when we send our notification during
                                                                     // block placement
            this.notify(blockposition);
        }
    ...
    public void a(int i, int j, int k, int l) {
        ...
        if (!this.worldProvider.o()) {
            ...
            for (i1 = k; chunk != null && i1 <= l; ++i1) {
                this.updateBrightness(EnumSkyBlock.SKY, new BlockPosition(i, i1, j), chunk); // PaperSpigot -
    ...
    public void applyPhysics(BlockPosition blockposition, Block block) {
        ...
        GlobalConfig.getInstance().getAntiXrayInstance().updateNearbyBlocks(this, blockposition); // Spigot
    }
    ...
    public final int getLight(BlockPosition blockposition, boolean checkNeighbors) {
    ...
    public BlockPosition getHighestBlockYAt(BlockPosition blockposition) {
        ...
        if (blockposition.getX() >= -30000000 && blockposition.getZ() >= -30000000 && blockposition.getX() < 30000000
            ...
            if (this.isChunkLoaded(blockposition.getX() >> 4, blockposition.getZ() >> 4, true)) {
                i = this.getChunkAt(blockposition.getX() >> 4, blockposition.getZ() >> 4).b(blockposition.getX() & 15,
        ...
        if (!Double.isNaN(vec3d.a) && !Double.isNaN(vec3d.b) && !Double.isNaN(vec3d.c)) {
            if (!Double.isNaN(vec3d1.a) && !Double.isNaN(vec3d1.b) && !Double.isNaN(vec3d1.c)) {
                ...
                while (k1-- >= 0) {
                    ...
                    // PandaSpigot start - ray tracing into an unloaded chunk should be treated as a
                    // miss, this saves a ton of lag for when AI tries to raytrace near unloaded
                    // chunks.
    ...
    public void makeSound(Entity entity, String s, float f, float f1) {
        ...
        // PandaSpigot end
        for (int i = 0; i < this.u.size(); ++i) {
            ((IWorldAccess) this.u.get(i)).a(s, loc.getX(), loc.getY(), loc.getZ(), f, f1); // PandaSpigot - Use x, y, z
                                                                                            // from event
        }
    ...
    public void a(EntityHuman entityhuman, String s, float f, float f1) {
        ...
        // PandaSpigot end
        for (int i = 0; i < this.u.size(); ++i) {
            ((IWorldAccess) this.u.get(i)).a(entityhuman, s, loc.getX(), loc.getY(), loc.getZ(), f, f1); // PandaSpigot
                                                                                                         // - Use x, y,
                                                                                                         // z from event
        }
    ...
    public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
        ...
        // Spigot start
            ...
            double radius = GlobalConfig.getInstance().getExpMerge();
            if (radius > 0) {
        ...
        if (entity instanceof EntityHuman) {
            ...
            for (Object o : worldMaps.c) {
                if (o instanceof WorldMap) {
                    ...
                    map.i.remove(entity);
                    for (Iterator<WorldMap.WorldMapHumanTracker> iter = (Iterator<WorldMap.WorldMapHumanTracker>) map.g
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
                    ...
                    for (int z = zstart; z < zend; z++) {
                        for (int y = ystart; y < l; y++) {
                            ...
                            if (block != null) {
                                ...
                                if (GlobalConfig.getInstance().isFallingBlocksCollideWithSigns()
                                        && (entity instanceof EntityTNTPrimed || entity instanceof EntityFallingBlock)
                                        && (b instanceof BlockSign || b instanceof BlockFenceGate
        ...
        if (entity instanceof EntityTNTPrimed)
            return arraylist; // PandaSpigot - Optimize tnt entity movement
    ...
    public void tickEntities() {
        ...
        for (i = 0; i < this.k.size(); ++i) {
            ...
            if (entity.getWorld() != this) {
                System.out.println("Attempted to tick entity " + entity + " in world " + entity.getWorld()
                        + " but was in world " + this);
                continue;
            }
        ...
        for (tileTickPosition = 0; tileTickPosition < tileEntityList.size(); tileTickPosition++) { // PaperSpigot -
                                                                                                   // Disable tick
                                                                                                   // limiters
            ...
            if (!tileentity.x() && tileentity.t()) {
                ...
                if (this.isLoaded(blockposition) && this.N.a(blockposition)) {
                    try {
                        ...
                        // PaperSpigot start - Prevent tile entity and entity crashes
                        String msg = "TileEntity threw exception at " + tileentity.world.getWorld().getName() + ":"
                                + tileentity.position.getX() + "," + tileentity.position.getY() + ","
                                + tileentity.position.getZ();
                        System.err.println(msg);
                        throwable2.printStackTrace();
                        getServer().getPluginManager()
                                .callEvent(new ServerExceptionEvent(new ServerInternalException(msg, throwable2)));
                        tilesThisCycle--;
                        this.tileEntityList.remove(tileTickPosition--);
                        continue;
                        // PaperSpigot end
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        ...
        if (!org.spigotmc.ActivationRange.checkIfActive(entity)) {
            ...
            if (!this.isChunkLoaded(i, j, true)
                    && ((entity instanceof EntityEnderPearl && GlobalConfig.getInstance().isRemoveUnloadedEnderPearls())
                            ||
                            (entity instanceof EntityFallingBlock
                                    && GlobalConfig.getInstance().isRemoveUnloadedFallingBlocks())
                            ||
                            (entity instanceof EntityTNTPrimed
                                    && GlobalConfig.getInstance().isRemoveUnloadedTNTEntities()))) {
                    ...
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (block.getMaterial() == material) {
                            double d0 = (double) ((float) (l1 + 1)
                                    - BlockFluids.b(((Integer) iblockdata.get(BlockFluids.LEVEL)).intValue()));
                        ...
                        : (block instanceof BlockStepAbstract
                                ...
                                : (block instanceof BlockHopper ? true
                                        : (block instanceof BlockSnow
                                                ? ((Integer) iblockdata.get(BlockSnow.LAYERS)).intValue() == 7
                                                : false))));
    }
    ...
    protected void D() {
        ...
        // Spigot start
        int optimalChunks = GlobalConfig.getInstance().getChunksPerTick();
        ...
        if (GlobalConfig.getInstance().isRandomLightUpdates() && !this.players.isEmpty()) { // Spigot
    ...
    protected void a(int i, int j, Chunk chunk) {
        ...
        if (!GlobalConfig.getInstance().isDisableMoodSounds() && this.L == 0 && !this.isClientSide) { // PaperSpigot -
            // Disable
            // mood sounds
            ...
            if (block.getMaterial() == Material.AIR && this.k(blockposition) <= this.random.nextInt(8)
                ...
                EntityHuman entityhuman = this.findNearbyPlayer((double) l + 0.5D, (double) j1 + 0.5D,
                        (double) i1 + 0.5D, 8.0D);

                if (entityhuman != null
                        && entityhuman.e((double) l + 0.5D, (double) j1 + 0.5D, (double) i1 + 0.5D) > 4.0D) {
                    this.makeSound((double) l + 0.5D, (double) j1 + 0.5D, (double) i1 + 0.5D, "ambient.cave.cave", 0.7F,
    ...
    public boolean c(EnumSkyBlock enumskyblock, BlockPosition blockposition, Chunk chunk, List<Chunk> neighbors) { // PaperSpigot
        ...
        if (chunk == null /* || !chunk.areNeighborsLoaded(1) */ /* !this.areChunksLoaded(blockposition, 17, false) */) {
            ...
            if (GlobalConfig.getInstance().isUseAsyncLighting()) {
    ...
    public boolean updateBrightness(EnumSkyBlock enumskyblock, BlockPosition position, Chunk chunk) {
        ...
        int x = position.getX();
        int z = position.getZ();
        // final Chunk chunk = this.getChunkIfLoaded(x >> 4, z >> 4); // PandaSpigot -
        // Moved up
    ...
    public int getBlockFacePower(BlockPosition blockposition, EnumDirection enumdirection) {
        ...
        return block.isOccluding() ? this.getBlockPower(blockposition)
                : block.a((IBlockAccess) this, blockposition, iblockdata, enumdirection);
    }
