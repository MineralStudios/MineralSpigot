...
public class WorldServer extends World implements IAsyncTaskHandler, Callable<WorldServer> {
    ...
    private int T;
    private static final List<StructurePieceTreasure> U = Lists.newArrayList(new StructurePieceTreasure[] {
    ...
    public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
        ...
        this.manager = new PlayerChunkMap(this, GlobalConfig.getInstance().getViewDistance()); // Spigot
        this.worldProvider.a(this);
        this.chunkProvider = this.k();
        this.Q = new org.bukkit.craftbukkit.CraftTravelAgent(this); // CraftBukkit // Paper - configurable search radius
                                                                    // // PandaSpigot - Just define the variable
                                                                    // directly
        this.B();
        this.C();
    ...
    public void doTick() {
        ...
        this.h();
        GlobalConfig.getInstance().getAntiXrayInstance().flushUpdates(this); // PaperSpigot
    ...
    public boolean everyoneDeeplySleeping() {
        if (this.O && !this.isClientSide) {
            ...
            // really, allowing time to pass in spite of AFKers
    ...
    protected void h() {
        ...
        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            for (ObjectIterator<Entry> iter = chunkTickList.long2ShortEntrySet().iterator(); iter.hasNext();) {
                ...
                if ((!this.chunkProvider.isChunkLoaded(chunkX, chunkZ))
                        || (this.chunkProviderServer.unloadQueue.contains(chunkX, chunkZ))) {
                    iter.remove();
                    continue;
                }
                // Spigot end
                ...
                if (!GlobalConfig.getInstance().isDisableThunder() && this.random.nextInt(100000) == 0 && this.S()
                        && this.R()) { // PaperSpigot - Disable thunder
                    this.m = this.m * 3 + 1013904223;
                    i1 = this.m >> 2;
                    blockposition = this.a(new BlockPosition(k + (i1 & 15), 0, l + (i1 >> 8 & 15)));
                    if (this.isRainingAt(blockposition)) {
                        this.strikeLightning(new EntityLightning(this, (double) blockposition.getX(),
                                (double) blockposition.getY(), (double) blockposition.getZ()));
                    }
                }

                this.methodProfiler.c("iceandsnow");
                if (!GlobalConfig.getInstance().isDisableIceAndSnow() && this.random.nextInt(16) == 0) { // PaperSpigot
                                                                                                         // -
        ...
        // Spigot Start
        if (GlobalConfig.getInstance().isClearChunksOnTick()) {
            chunkTickList.clear();
        }
        // Spigot End
    }
    ...
    protected BlockPosition a(BlockPosition blockposition) {
        ...
        AxisAlignedBB axisalignedbb = (new AxisAlignedBB(blockposition1,
                new BlockPosition(blockposition1.getX(), this.getHeight(), blockposition1.getZ())))
                .grow(3.0D, 3.0D, 3.0D);
        List<EntityLiving> list = this.a(EntityLiving.class, axisalignedbb, new Predicate<EntityLiving>() {
    ...
    public void tickEntities() {
        ...
        super.tickEntities();
        GlobalConfig.getInstance().currentPrimedTnt = 0; // Spigot
    }
    ...
    public boolean a(boolean flag) {
        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            /*
             * if (i > 1000) {
             * // CraftBukkit start - If the server has too much to process over time, try
             * to alleviate that
             * if (i > 20 * 1000) {
             * i = i / 20;
             * } else {
             ...
             * // CraftBukkit end
             */
            if (i > GlobalConfig.getInstance().getTickNextTickCap()) {
                i = GlobalConfig.getInstance().getTickNextTickCap();
            }
            // PaperSpigot end
            ...
            if (GlobalConfig.getInstance().isTickNextTickListCapIgnoresRedstone()) {
     ...
     * if (!this.getSpawnAnimals() && (entity instanceof EntityAnimal || entity
     * instanceof EntityWaterAnimal)) {
     * entity.die();
     * }
     * 
    ...
    public void save(boolean flag, IProgressUpdate iprogressupdate) throws ExceptionWorldConflict {
        if (this.chunkProvider.canSave()) {
            ...
            if (!GlobalConfig.getInstance().isDisableChunkSaving()) { // PandaSpigot - Configurable chunk saving
    ...
    public Explosion createExplosion(Entity entity, double d0, double d1, double d2, float f, boolean flag,
        ...
        /*
         * Remove
         * Explosion explosion = new Explosion(this, entity, d0, d1, d2, f, flag,
         * flag1);
         * 
         * explosion.a();
         * explosion.a(false);
        ...
        while (!this.S[this.T].isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (this.a(blockactiondata)) {
                    ...
                    this.server.getPlayerList().sendPacketNearby((double) blockactiondata.a().getX(),
                            (double) blockactiondata.a().getY(), (double) blockactiondata.a().getZ(), 64.0D, dimension,
    ...
    protected void p() {
         ...
         * CraftBukkit start
         * if (this.o != this.p) {
         * this.server.getPlayerList().a(new PacketPlayOutGameStateChange(7, this.p),
         * this.worldProvider.getDimension());
         * }
         * 
         * if (this.q != this.r) {
         * this.server.getPlayerList().a(new PacketPlayOutGameStateChange(8, this.r),
         * this.worldProvider.getDimension());
         * }
         * 
         * if (flag != this.S()) {
         ...
         * this.p));
         * this.server.getPlayerList().sendAll(new PacketPlayOutGameStateChange(8,
         * this.r));
         * }
         * //
         */
    ...
    @Override
    public WorldServer call() throws Exception {
        ...
        this.getTracker().updatePlayers();
        this.explosionDensityCache.clear(); // PaperSpigot - Optimize explosions
        return this;
    }
}
