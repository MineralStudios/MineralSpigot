 ...
 * They are never to be accessed outside of the main thread; alternatives exist
 * to prevent locking.</li>
 * <li>{@link #head} to {@link #tail} act as a linked list/queue, with 1
 * consumer and infinite producers.
 * Adding to the tail is atomic and very efficient; utility method is
 * {@link #handle(CraftTask, long)} or {@link #addTask(CraftTask)}.</li>
 * <li>Changing the period on a task is delicate.
 * Any future task needs to notify waiting threads.
 * Async tasks must be synchronized to make sure that any thread that's
 * finishing will remove itself from {@link #runners}.
 * Another utility method is provided for this,
 * {@link #cancelTask(CraftTask)}</li>
 ...
 * perform any data changes on the main thread.
 * When executed from inside a synchronous method, the scheduler will be updated
 * before next execution by virtue of the frequent {@link #parsePending()}
 * calls.</li>
 */
public class CraftScheduler implements BukkitScheduler {
     ...
     * {@link CraftTask#next} is the live reference.
     */
