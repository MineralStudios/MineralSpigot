...
public class AntiXray {
    ...
    public AntiXray(int engineMode, int[] hiddenBlocks, int[] replaceBlocks) {
        ...
        for (int id : (engineMode == 1) ? hiddenBlocks : replaceBlocks) {
            obfuscateBlocks[id] = true;
        }

        // For every block
        TByteSet blocks = new TByteHashSet();
        for (Integer i : hiddenBlocks) {
    ...
    public void flushUpdates(World world) {
        if (GlobalConfig.getInstance().isAntiXray() && !pendingUpdates.isEmpty()) {
            queueUpdates = false;
    ...
    public void updateNearbyBlocks(World world, BlockPosition position) {
        if (GlobalConfig.getInstance().isAntiXray()) {
            // PaperSpigot start
            if (queueUpdates) {
                        ...
                        for (int z = 0; z < 16; z++) {
                            for (int x = 0; x < 16; x++) {
                                ...
                                if (obfuscateBlocks[blockId]) {
                                    ...
                                    if (!isLoaded(world, new BlockPosition(startX + x, (i << 4) + y, startZ + z),
                                            initialRadius)) {
                                        index++;
                                        continue;
                                    }
                                    ...
                                    if (!hasTransparentBlockAdjacent(world,
                                            new BlockPosition(startX + x, (i << 4) + y, startZ + z), initialRadius)) {
                                        int newId = blockId;
                                        switch (GlobalConfig.getInstance().getEngineMode()) {
                                            case 1:
