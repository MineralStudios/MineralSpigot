...
public class ItemFactoryTest extends AbstractTestingBase {
    ...
    public void testKnownAttributes() throws Throwable {
        final ZipInputStream nmsZipStream = new ZipInputStream(
                CommandAbstract.class/* Magic class that isn't imported! */.getProtectionDomain().getCodeSource()
        ...
        for (ZipEntry clazzEntry; (clazzEntry = nmsZipStream.getNextEntry()) != null;) {
            ...
            final Class<?> clazz = Class
                    .forName(entryName.substring(0, entryName.length() - ".class".length()).replace('/', '.'));
            ...
            for (final Field field : clazz.getDeclaredFields()) {
                if (IAttribute.class.isAssignableFrom(field.getType()) && Modifier.isStatic(field.getModifiers())) {
                    ...
                    assertThat("Logical error: duplicate name `" + attributeName + "' in " + clazz.getName(),
                            names.add(attributeName), is(true));
